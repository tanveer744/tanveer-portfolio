...
export default function Notepad({ windowData }) {
  ...
  const downloadResume = (format = 'pdf') => {
    ...
    link.href = format === 'pdf' ? '/resume/Shaik_Tanveer_Lohare_Resume.pdf' : '/resume/Shaik_Tanveer_Lohare_Resume.docx'
    link.download = `Shaik_Tanveer_Lohare_Resume.${format}`
    ...
    <div className="w-full h-full flex flex-col bg-white">
      ...
      {currentNote && (
        <div className="h-9 bg-blue-50 border-b border-blue-100 flex items-center px-4 gap-3">
          ...
          {/* Download Resume Button - Only show for About Me */}
          {currentFile === 'about-me' && (
            <div className="flex items-center gap-2">
              <button
                onClick={() => downloadResume('pdf')}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-win-accent hover:bg-win-accent-hover text-white text-xs font-medium rounded transition-colors"
