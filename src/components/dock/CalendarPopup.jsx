...
export default function CalendarPopup({ onClose }) {
  ...
  return (
    ...
    >
      <div className="bg-gray-900/95 backdrop-blur-xl rounded-xl border border-white/10 shadow-2xl overflow-hidden w-[320px]">
        ...
        <div className="flex items-center justify-between px-4 py-3">
          ...
          >
            <IoChevronBack className="w-5 h-5 text-white/70" />
          </button>
          <button
            onClick={goToToday}
          ...
          <button
            ...
            className="p-1.5 rounded-full hover:bg-white/10 transition-colors"
          >
            <IoChevronForward className="w-5 h-5 text-white/70" />
          </button>
        </div>
        ...
        <div className="grid grid-cols-7 px-3">
          {dayNames.map((day) => (
            <div key={day} className="w-9 h-8 flex items-center justify-center text-white/50 text-xs font-medium">
              {day}
            </div>
          ))}
        </div>
